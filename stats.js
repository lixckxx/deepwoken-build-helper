document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayResults(solution) {
        ...
        // First, show all stats that went through shrine averaging
        for (const [stat, postValue] of Object.entries(solution.postShrine)) {
            const finalValue = solution.finalStats[stat] || postValue;
        ...
        // Then, add stats that were invested purely post-shrine (not in shrine at all)
        for (const [stat, finalValue] of Object.entries(solution.finalStats)) {
            if (!solution.postShrine[stat] && finalValue > 0) {
                const statItem = document.createElement('div');
                statItem.className = 'stat-item';
                statItem.innerHTML = `
            <span class="stat-name">${stat}</span>
